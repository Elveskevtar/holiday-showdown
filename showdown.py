...
def upgrade(upgrade_type):
	...
	if upgrade_type in const.ITEM_STRINGS:
		...
		return question('Successfully upgraded advertising to ' +
						const.ADVERTISING[ad_level][1])
	return question('Could not recognize upgrade option')
...
def action(action_type, sabotage_target):
	...
	if session.attributes['has_done'][player]:
		return question('You have already performed an action this turn')
	...
	if action_type in const.SABOTAGE_STRINGS:
		if sabotage_target not in session.attributes['player_names']:
			return question('Could not recognize sabotage target')
		target = session.attributes['player_names'].index(sabotage_target)
		if player == target:
			return question('Cannot sabotage yourself')
		...
		if sabotage[1] == 'spread rumors':
			...
			session.attributes['has_done'][player] = True
			return question(session.attributes['player_names'][player] + ' ads generated '
							' negative publicity for ' +
							session.attributes['player_names'][target] + 'shop')
		elif sabotage[1] == 'hire spy':
			if session.attributes['store_level'][target] == 1:
				return question('Cannot sabotage. Target\'s store too low')
			...
			return question(session.attributes['player_names'][player] + ' hired a spy '
							...
							' is forced into a store downgrade.')
		elif sabotage[1] == 'steal items':
			if session.attributes['item_level'][target] == 1:
				return question('Cannot sabotage. Target\'s items too low')
			...
			return question(session.attributes['player_names'][player] + ', against better '
							...
							' is forced into an item downgrade.')
		return question('Could not recognize sabotage type')
	return question('Could not recognize action type')
...
def get_stat(stat):
	...
	if stat in const.COOKIE_STRINGS:
		...
		return question('You current advertisment is ' + ad[1] + ' with a multiplier range '
						'from ' + str(ad[3]) + ' to ' + str(ad[4]))
	return question('Could not recognize what you are asking for')
